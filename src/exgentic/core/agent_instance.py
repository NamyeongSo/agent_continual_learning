...
class AgentInstance(ABC):
    ...
    def receive_training_feedback(self, feedback: dict[str, Any]) -> None:
        """Receive official post-session grading for opt-in training-time agents.

        Called after ``Session.score()`` and before ``close()``. The default
        implementation keeps agents without training-time learning unchanged.
        """
        return None
