...
# Training-time reflection sees the benchmark's official post-session score.
# The default prompt above remains unchanged for test-time ACE experiments.
REFLECTOR_PROMPT_TRAINING_TIME = (
    REFLECTOR_PROMPT_NO_GT
    .replace(
        "based on the reasoning trace alone.",
        "using the reasoning trace and official post-session grading feedback.",
    )
    .replace(
        "- Carefully analyze the model's reasoning trace to evaluate its approach",
        "- Carefully analyze the model's reasoning trace to evaluate its approach\n"
        "- Treat official grading feedback as ground truth about the outcome. "
        "A successful submission is not necessarily a full score; use the score "
        "and detailed test or checker results when diagnosing mistakes.",
    )
    .replace(
        "**Full Playbook:**",
        "**Official Post-Session Grading Feedback (not visible during task execution):**\n"
        "{grading_feedback}\n\n"
        "**Full Playbook:**",
    )
)
