...
Environment variables:
    ...
    EXGENTIC_LLM_TRACE_FORMAT: set to ``dedup_v1`` to store repeated request
        messages and tool definitions once and reference them by content hash.
...
TRACE_FORMAT_ENV = "EXGENTIC_LLM_TRACE_FORMAT"
...
class TraceLogger(CustomLogger):
    def __init__(self, file_path: str | None = None) -> None:
        ...
        self._written_definitions: dict[str, set[str]] = {}
    ...
    def _write_row(self, kwargs: dict[str, Any], response_obj: dict[str, Any], status: str) -> None:
        ...
        if os.environ.get(TRACE_FORMAT_ENV) == "dedup_v1":
            with self._write_lock:
                definitions = self._compact_request(file_path, row)
                with open(file_path, "a", encoding="utf-8") as f:
                    for definition in definitions:
                        f.write(json.dumps(definition, ensure_ascii=False, default=str) + "\n")
                    f.write(json.dumps(row, ensure_ascii=False, default=str) + "\n")
                self._written_definitions.setdefault(file_path, set()).update(
    ...
    def _compact_request(self, file_path: str, row: dict[str, Any]) -> list[dict[str, Any]]:
        """Replace repeated request content with reconstructible content-addressed refs."""
        request = row["request"]
        seen = self._written_definitions.get(file_path, set())
        definitions: list[dict[str, Any]] = []
        pending: set[str] = set()

        def add_definition(kind: str, value: Any) -> str:
            encoded = json.dumps(value, ensure_ascii=False, sort_keys=True, default=str)
            definition_id = f"{kind}:{hashlib.sha256(encoded.encode('utf-8')).hexdigest()}"
            if definition_id not in seen and definition_id not in pending:
                definitions.append({
                    "record_type": "definition",
                    "id": definition_id,
                    "value": json.loads(encoded),
                })
                pending.add(definition_id)
            return definition_id

        for field in ("messages", "tools", "functions"):
            values = request.get(field)
            if isinstance(values, list):
                request[f"{field}_refs"] = [add_definition(field, value) for value in values]
                del request[field]

        if isinstance(request.get("prompt"), str):
            request["prompt_ref"] = add_definition("prompt", request.pop("prompt"))

        row["trace_format"] = "dedup_v1"
        return definitions
...
def iter_expanded_trace_rows(log_path: str | Path):
    """Yield LLM call rows with ``dedup_v1`` request content reconstructed."""
    ...
    with open(log_path, encoding="utf-8") as log_file:
        for line in log_file:
            if not line.strip():
                continue
            row = json.loads(line)
            if row.get("record_type") == "definition":
                definitions[row["id"]] = row["value"]
                continue
            if row.get("trace_format") == "dedup_v1":
                request = row["request"]
                for field in ("messages", "tools", "functions"):
                    refs = request.pop(f"{field}_refs", None)
                    if refs is not None:
                        request[field] = [definitions[ref] for ref in refs]
                prompt_ref = request.pop("prompt_ref", None)
                if prompt_ref is not None:
                    request["prompt"] = definitions[prompt_ref]
            yield row
